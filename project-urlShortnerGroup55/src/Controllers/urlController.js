...
const redisClient = redis.createClient(
    14549,
    "redis-14549.c264.ap-south-1-1.ec2.cloud.redislabs.com",
    { no_ready_check: true }
  );
  redisClient.auth("iZt5JRBM45fedzLkDORAWX7eXekiGMBQ", function (err) {
    if (err) throw err;
...
const getUrlCode = async function (req, res) {
    try {
        ...
        if(cachedUrl)    {
            return res.status(302).redirect((cachedUrl.startsWith("http") ? "" :"https://") + cachedUrl)
        }
        ...
        if (url) {
            ...
            return res.status(302).redirect((url.longUrl.startsWith("http") ? "" :"https://") + url.longUrl)
        } else {
